...
public class TestView : MonoBehaviour
{
    ...
    void Start()
    {
        //_mapContainer = GameObject.Find("Map").GetComponent<Map>();

        int collumns = _mapContainer.Map.Cols;
        int rows = _mapContainer.Map.Rows;
    ...
    private void DrawMap(int lines, int collumns)
    {
        for(int i = 0; i < lines; i++)
        {
            for(int y = 0; y < collumns; y++)
            {
                ...
                Tile newTile = _mapContainer.Map.GetTile(i,y);
